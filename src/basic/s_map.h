...
class ConstIter
	...
	private:
		typedef typename pair_vec_t::const_iterator iter_type_t; 

		iter_type_t _it;

	public:
				 ConstIter() : _it() { }
				 ConstIter(const ConstIter & iter) : _it(iter._it) { }
				 ConstIter(const iter_type_t & it) : _it(it) { }
		ConstIter &	 operator = (const ConstIter & iter) { _it = iter._it; return *this; }
				~ConstIter() { }

		Pair	 operator * () { return *_it; }
		Pair	 operator -> () { return *_it; }
		bool	 operator == (const ConstIter & iter) const { return this->_it == iter._it; }
		bool	 operator != (const ConstIter & iter) const { return this->_it != iter._it; }
		ConstIter &	 operator ++ () { ++_it; return *this; }
		ConstIter &	 operator -- () { --_it; return *this; }
		ConstIter &	 operator += (int n) { _it += n; return *this; }
		ConstIter &	 operator -= (int n) { _it -= n; return *this; }
		ConstIter &	 operator + (int n) { ConstIter new_it(*this); new_it += n; return new_it; }
		ConstIter &	 operator - (int n) { ConstIter new_it(*this); new_it -= n; return new_it; }
	};
...
public:
	...
	bool	 find(const KeyT & key, ConstIter * it_ptr = nullptr) const;
