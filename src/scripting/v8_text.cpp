...
#define DEFINE_HANDLE_SCOPE_AND_GET_SELF_FOR_TEXT(that) DEFINE_HANDLE_SCOPE_AND_GET_SELF(Text, that)
...
{
	DEFINE_HANDLE_SCOPE_AND_GET_SELF_FOR_TEXT(info);
...
void JsText::JS_GetWidth(v8::Local<v8::String> name, const v8::PropertyCallbackInfo<v8::Value>& info)
{
	DEFINE_HANDLE_SCOPE_AND_GET_SELF_FOR_TEXT(info);
	info.GetReturnValue().Set(Number::New(info.GetIsolate(), self->getLocalBounds().width));
}

void JsText::JS_GetHeight(v8::Local<v8::String> name, const v8::PropertyCallbackInfo<v8::Value>& info)
{
	DEFINE_HANDLE_SCOPE_AND_GET_SELF_FOR_TEXT(info);
	info.GetReturnValue().Set(Number::New(info.GetIsolate(), self->getLocalBounds().height));
}
