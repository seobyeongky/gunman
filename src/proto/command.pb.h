...
const CommandType CommandType_MAX = COMMAND_SKILL;
...
// optional .Vector2d pos = 3;
inline bool Command::has_pos() const {
  return (_has_bits_[0] & 0x00000004u) != 0;
}
inline void Command::set_has_pos() {
  _has_bits_[0] |= 0x00000004u;
}
inline void Command::clear_has_pos() {
  _has_bits_[0] &= ~0x00000004u;
}
inline void Command::clear_pos() {
  if (pos_ != NULL) pos_->::Vector2d::Clear();
  clear_has_pos();
}
inline const ::Vector2d& Command::pos() const {
  return pos_ != NULL ? *pos_ : *default_instance_->pos_;
}
inline ::Vector2d* Command::mutable_pos() {
  set_has_pos();
  if (pos_ == NULL) pos_ = new ::Vector2d;
  return pos_;
}
inline ::Vector2d* Command::release_pos() {
  clear_has_pos();
  ::Vector2d* temp = pos_;
  pos_ = NULL;
  return temp;
}
inline void Command::set_allocated_pos(::Vector2d* pos) {
  delete pos_;
  pos_ = pos;
  if (pos) {
    set_has_pos();
